...
Add these to your `~/.zshrc` or `~/.bashrc`. **Do this before running any Pulumi or AWS commands** — without them you'll get `NoCredentialProviders` errors on every command.

```bash
echo 'export AWS_PROFILE=sable-dev' >> ~/.zshrc
echo "export PULUMI_BACKEND_URL='s3://sable-infra-state?region=us-east-1'" >> ~/.zshrc
echo 'export PULUMI_CONFIG_PASSPHRASE="get-this-from-1password"' >> ~/.zshrc
source ~/.zshrc
```

Replace `get-this-from-1password` with the actual passphrase from 1Password.

Verify they're set:
```bash
echo $AWS_PROFILE           # should print: sable-dev
echo $PULUMI_BACKEND_URL    # should print: s3://sable-infra-state?region=us-east-1
```

> If you open a new terminal and commands start failing with `NoCredentialProviders`, it means the env vars didn't get saved. Check `~/.zshrc` has the exports and run `source ~/.zshrc`.
...
**`AWS_PROFILE` and `PULUMI_BACKEND_URL` must be set in your shell**
If you get `NoCredentialProviders` errors, your shell env vars aren't set. Re-run step 7 of the first-time setup. Opening a new terminal without sourcing `~/.zshrc` is the most common cause.
...
> No need to specify `--profile` — `AWS_PROFILE=sable-dev` is set in your shell.
...
> No need to specify the URL — `PULUMI_BACKEND_URL` is set in your shell.
...
One RDS instance per stack, provisioned in `database.ts`.

| Setting | Value |
|---------|-------|
| Engine | Postgres 16 |
| Instance class | `db.t4g.small` (both stacks) |
| Storage | 20 GB gp3, encrypted |
| Multi-AZ | Off (MVP) |
| Public access | Off |
| Backup retention | 7 days |
| Snapshot window | 03:00–04:00 UTC |
| Deletion protection | Off on dev, on on prod |

### Credentials

The master password is randomly generated by Pulumi and written to Secrets Manager at:

```
sable/<stack>/db/credentials
```

The secret is a JSON object:
...
```json
{
  ...
  "password": "...",
  "url": "postgresql://sable_admin:...@...:5432/sable"
}
```
...
aws secretsmanager get-secret-value \
  --secret-id sable/dev/db/credentials \
  --query SecretString \
  --output text | jq .
```
...
| Output | Used by |
|--------|---------|
| `dbEndpoint` | App Runner env vars (KAN-44), local `.env` |
| `dbPort` | App Runner env vars |
| `dbSecretArn` | IAM policy for deploy role and App Runner task role |
| `dbSecretName` | CI workflow — fetches `DATABASE_URL` before migrations |

### Updating the instance class

```bash
pulumi config set sable-infra:dbInstanceClass db.t4g.medium --stack prod
pulumi up --stack prod
```

RDS will apply the change during the next maintenance window unless you force it immediately.

---

## CI — Prisma migrations

Migrations run automatically on every deploy via `.github/workflows/backend-deploy.yml`.

**Job order:**
1. `prisma-migrate` — fetches `DATABASE_URL` from Secrets Manager, runs `npx prisma migrate deploy`
2. `db-smoke-test` — dev only, asserts `SELECT 1` works after migration
3. `deploy-app-runner` — only runs if migrations succeed

If migrations fail, the App Runner deploy never happens.

### Running migrations locally

```bash
# Get the dev DATABASE_URL
...
| Secret | Description |
|--------|-------------|
| `AWS_DEPLOY_ROLE_DEV` | ARN of the IAM role CI assumes for dev deploys |
| `AWS_DEPLOY_ROLE_PROD` | ARN of the IAM role CI assumes for prod deploys |
| `PULUMI_CONFIG_PASSPHRASE` | Pulumi secrets passphrase |

These are set in GitHub → Settings → Secrets → Actions.
