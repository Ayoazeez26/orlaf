...
import type { PayoutAnalyticsMetric } from "../types"

interface PayoutsAnalyticsMetricsProps {
  metrics: PayoutAnalyticsMetric[]
}

function isTrendPositive(metric: PayoutAnalyticsMetric) {
  const increased = metric.changePercent >= 0
  return metric.increaseIsGood ? increased : !increased
}

export function PayoutsAnalyticsMetrics({
  metrics,
}: PayoutsAnalyticsMetricsProps) {
  return (
