...
import type { SubscriptionAnalyticsMetric } from "../types"

interface SubscriptionsAnalyticsMetricsProps {
  metrics: SubscriptionAnalyticsMetric[]
}

function isTrendPositive(metric: SubscriptionAnalyticsMetric) {
  const increased = metric.changePercent >= 0
  return metric.increaseIsGood ? increased : !increased
}

export function SubscriptionsAnalyticsMetrics({
  metrics,
}: SubscriptionsAnalyticsMetricsProps) {
  return (
